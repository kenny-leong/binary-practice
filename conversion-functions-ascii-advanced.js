...
function binaryToHexadecimal(blob) {
  let hexa = parseInt(blob.slice(2), 2).toString(16);
  return `0x${hexa}`
}

/* Base 16 to base 2 */
function hexadecimalToBinary(blob) {
  let bina = parseInt(blob.slice(2), 16).toString(2);
  return `0b${bina}`
}
...
function binaryToAscii(blob) {
  let int = parseInt(blob.slice(2), 2)
  return String.fromCharCode(int);
}

/* Base 16 to ASCII */
function hexadecimalToAscii(blob) {
  let int = parseInt(blob.slice(2), 16);
  return String.fromCharCode(int);
}

/* ASCII to base 10 */
function asciiToDecimal(blob) {
  let strList = blob.split('');
  let arr = [];
  strList.forEach(element => {
    arr.push(element.charCodeAt());
  });
  return arr;
}
